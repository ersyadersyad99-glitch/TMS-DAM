...
router.get('/provinces', requireAuth, async (_req, res, next) => {
  try {
...
router.get('/cities', requireAuth, async (req, res, next) => {
  try {
    ...
    const data = await locationsService.getCities(province);
    res.json(data);
  } catch (err) { next(err); }
});
...
router.get('/stores', requireAuth, async (req, res, next) => {
  try {
    ...
    const data = await locationsService.getStores(city);
    res.json(data);
  } catch (err) { next(err); }
});
...
router.post('/', requireAuth, requireRole('admin'), async (req, res, next) => {
  try {
    ...
    const loc = await locationsService.create(parsed.data);
